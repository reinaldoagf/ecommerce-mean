...
module.exports.details = async (req, res, next) => {
  try {
    const doc = await Product.findOne({ _id: req.params.id }).populate("user");
    if (!doc)
      return res
        .status(404)
...
module.exports.all = async (req, res, next) => {
  try {
    const doc = await Product.find(
      {},
      { brand: 1, model: 1, price: 1, stock: 1 }
    ).populate("user");
    if (!doc)
      return res
        .status(404)
...
module.exports.delete = async (req, res, next) => {
  try {
    const doc = await Product.remove({ _id: req.params.id });
    if (!doc)
      return res
        .status(404)
        .json({ status: false, message: "Product not founds." });
